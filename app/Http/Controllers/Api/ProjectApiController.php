...
class ProjectApiController extends Controller
{
    ...
    public function index(Request $request): ProjectCollection
    {
        ...
        $paginator = WorkspaceAccess::scopeProjects(Project::query(), $request->user())
            ->with([
                'client:id,full_name',
                'user:id,name',
                'objectDetails.client:id,full_name',
                'object:id,address,city,type,apartment,apartment_floor,apartment_entrance,area,latitude,longitude,client_id',
                'stages.steps',
            ])
            ->withCount('stages')
            ->orderByDesc('id')
        ...
        $this->attachPipelineStages($request->user(), $paginator->getCollection());
    ...
    public function store(StoreProjectRequest $request): JsonResponse
    {
        ...
        return (new ProjectResource($this->load($request->user(), $project)))
            ->response()
            ->setStatusCode(201);
    }
    ...
    public function show(Request $request, int $projectId): ProjectResource
    {
        return new ProjectResource(
            $this->load($request->user(), $this->projects->findAccessible($request->user(), $projectId))
        );
    }
    ...
    public function activity(Request $request, int $projectId)
    {
        ...
        return ActivityEventResource::collection(
            $this->activity->forSubject('project', $project->id, $request->query('filter'), $request->integer('per_page', 30))
        );
    }
    ...
    public function comments(Request $request, int $projectId)
    {
        ...
        return ActivityEventResource::collection(
            $this->activity->forSubject('project', $project->id, 'comments', $request->integer('per_page', 30))
        );
    }
    ...
    public function storeComment(StoreProjectCommentRequest $request, int $projectId): ActivityEventResource
    {
        ...
        return new ActivityEventResource(
            $this->activity->record(
                $project->user_id,
                'project',
                $project->id,
    ...
    private function load(User $user, Project $project): Project
    {
        $fresh = $project->fresh([
            'client:id,full_name',
            'user:id,name',
        ...
        $this->attachPipelineStages($user, collect([$fresh]));

        return $fresh;
    }

    /**
     * @param  Collection<int, Project>|LengthAwarePaginator<int, Project>  $projects
     */
    private function attachPipelineStages(User $user, Collection $projects): void
    {
        if ($projects->isEmpty()) {
            return;
        }

        $this->pipelines->ensureDefaultsForUser($user->id);
        $pipeline = Pipeline::defaultForUser($user->id, PipelineType::Project);
        /** @var Collection<string, PipelineStage> $byKey */
        $byKey = $pipeline->stages()->get()->keyBy('system_key');

        foreach ($projects as $project) {
            $project->setRelation('pipelineStage', $byKey->get((string) $project->status));
        }
    }
}
