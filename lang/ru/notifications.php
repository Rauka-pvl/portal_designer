<?php

return [
    'title' => 'Уведомления',
    'subtitle' => 'Изменения статусов от модератора',
    'subtitle_supplier' => 'Новые поставки и изменения заказов от дизайнеров',
    'empty' => 'Пока уведомлений нет.',
    'empty_unread' => 'Непрочитанных уведомлений нет.',
    'read' => 'Прочитано',
    'unread' => 'Непрочитанные',
    'filter_all' => 'Все',
    'filter_unread' => 'Непрочитанные',
    'mark_read' => 'Отметить прочитанным',
    'mark_unread' => 'Отметить непрочитанным',
    'mark_all_read' => 'Прочитать все',
    'marked_read' => 'Уведомление отмечено как прочитанное.',
    'marked_unread' => 'Уведомление отмечено как непрочитанное.',
    'marked_all_read' => 'Все уведомления отмечены как прочитанные.',
    'delete' => 'Удалить уведомление',
    'deleted' => 'Уведомление удалено.',
    'delete_confirm' => 'Удалить это уведомление?',
    'menu_actions' => 'Действия с уведомлением',
    'order_unavailable' => 'Заказ больше недоступен',

    'status_approved' => 'Одобрено',
    'status_rejected' => 'Отклонено',

    'supplier_title' => 'Поставщик: :status',
    'supplier_comment' => 'Модератор изменил статус поставщика ":name".',

    'object_title' => 'Объект: :status',
    'object_comment' => 'Модератор изменил статус объекта ":name".',
    'referral_supplier_title' => 'Поставщик по реферальной ссылке',
    'referral_supplier_comment' => 'Новый поставщик ":name" отправлен по вашей реферальной ссылке. Нажмите «Добавить поставщика», чтобы включить его в список.',
    'referral_supplier_add' => 'Добавить поставщика',
    'add_supplier' => 'Добавить поставщика',
    'view_supplier' => 'Посмотреть поставщика',
    'view_supplier_short' => 'Посмотреть',
    'referral_supplier_confirmed' => 'Поставщик добавлен',
    'action_unavailable' => 'Это действие больше недоступно.',
    'supplier_missing' => 'Поставщик не найден.',
    'accept_team_invite' => 'Принять',
    'decline_team_invite' => 'Отклонить',
    'team_invite_accepted' => 'Вы вступили в команду.',
    'team_invite_declined' => 'Приглашение отклонено.',
    'supplier_invite_title' => 'Новое приглашение от дизайнера',
    'supplier_invite_comment' => 'Дизайнер :name приглашает вас к сотрудничеству. Подтвердите приглашение в кабинете поставщика.',
    'new_order_title' => 'Новая поставка',
    'new_order_comment' => 'Вам отправлена новая поставка №:order.',
    'order_withdrawn_title' => 'Поставка изменена',
    'order_withdrawn_comment' => ':designer изменил(а) поставку №:order. Заказ сейчас не отправлен и сохранён как черновик.',
    'order_updated_title' => 'Изменены данные заказа',
    'order_updated_comment' => 'Дизайнер :designer изменил данные заказа №:order по проекту :project.',
    'offer_new_title' => 'Новое предложение по поставке',
    'offer_new_comment' => 'Дизайнер отправил предложение по поставке №:order (бонус :percent%). Примите, отклоните или предложите свой процент.',
    'offer_accepted_title' => 'Предложение принято',
    'offer_accepted_comment' => 'Предложение по поставке №:order принято. Бонус :percent%. Поставка активна.',
    'offer_rejected_title' => 'Предложение отклонено',
    'offer_rejected_comment' => 'Предложение по поставке №:order отклонено.',
    'offer_counter_title' => 'Встречное предложение',
    'offer_counter_comment' => ':by предложил(а) :percent% по поставке №:order. Ответьте: принять, отклонить или предложить свой процент.',
    'offer_view_order' => 'К заказу',
    'to_order' => 'К заказу',
    'to_orders' => 'К заказам',
    'to_post' => 'К публикации',
    'to_object' => 'К объекту',
    'rate_supplier_title' => 'Оцените поставщика',
    'rate_supplier_comment' => 'Поставка №:order завершена. Пожалуйста, оцените работу поставщика :supplier.',
    'rate_designer_title' => 'Оцените дизайнера',
    'rate_designer_comment' => 'Поставка №:order завершена. Пожалуйста, оцените работу дизайнера :designer.',
    'rate_action' => 'Оценить',
    'rate_supplier_action' => 'Оценить поставщика',
    'rate_designer_action' => 'Оценить дизайнера',
    'rate_sent' => 'Оценка отправлена',
    'community_like_title' => 'Новая оценка публикации',
    'community_like_comment' => ':name оценил(а) вашу публикацию',
    'community_like_comment_grouped' => ':name и ещё :count пользователей оценили вашу публикацию',
    'community_comment_title' => 'Новый комментарий',
    'community_comment_comment' => ':name прокомментировал(а) вашу публикацию',
    'community_reply_title' => 'Ответ на комментарий',
    'community_reply_comment' => ':name ответил(а) на ваш комментарий',
    'community_view_post' => 'К публикации',
];
