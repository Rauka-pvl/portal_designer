...
@section('content')
                            ...
                            <tr class="hover:bg-[#f8fafc] dark:hover:bg-[#0a0a0a]"
                                ...
                                <td class="px-4 py-3 text-sm">
                                    <div class="flex items-center gap-2">
                                        ...
                                        <form id="delete-client-{{ $client->id }}" method="POST"
                                            action="{{ route('clients.delete_client', $client->id) }}">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit"
            ...
            <form id="client-form" method="POST" action="{{ route('clients.add_client') }}"
                ...
                <div class="flex-1 overflow-y-auto px-6 py-5 space-y-5">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                        <div>
                            <label class="modal-label modal-label-required">{{ __('clients.full_name') }}</label>
    ...
    <div id="object-modal"
        ...
        <div class="bg-white dark:bg-[#161615] rounded-xl max-w-2xl w-full mx-auto max-h-[90vh] overflow-hidden flex flex-col modal-content border border-[#e2e8f0] dark:border-[#3E3E3A]"
            ...
            <form id="object-form" class="flex flex-col flex-1 min-h-0" method="POST" action={{ route('objects.add_object') }} enctype="multipart/form-data">
                @csrf
                <div class="flex-1 overflow-y-auto px-6 py-5 space-y-5">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                        <div>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            window.renderFunnel = function() {
                ...
                Object.keys(clientsByStatus).forEach(status => {
                    ...
                    container.innerHTML = clientsByStatus[status].map(client => `
    ...
    `;
        ...
        function updateClientStatus(clientId, newStatus, updatedClient) {
            ...
            fetch(`/clients/${numericId}/status`, {
                ...
                .then(data => {
                    if (data.success && updatedClient) {
                        updateClientStatusInAllViews(numericId, newStatus, updatedClient);
                        const idx = (window.allClients || []).findIndex(c => c.id === numericId);
                        if (idx >= 0) window.allClients[idx].status = newStatus;
                    }
                    draggedElement = null;
                    if (!data.success) {
        ...
        function deleteClient(id) {
            ...
            fetch('{{ url("clients/delete") }}/' + id, {
                ...
                if (r.ok) {
                    document.querySelectorAll(`tr[data-client], div[data-client]`).forEach(el => {
                        try {
                            const c = JSON.parse(el.getAttribute('data-client') || '{}');
                            if (c.id === id) el.remove();
                        } catch (_) {}
                    });
                    document.querySelectorAll(`.funnel-card[data-client-id="${id}"]`).forEach(el => el.remove());
                    if (window.allClients) window.allClients = window.allClients.filter(c => c.id != id);
                    return;
                }
                throw new Error('Delete failed');
            }).catch(() => alert('{{ __("clients.error") }}'));
        }
