{{--
    LEGACY BACKUP — kept only for reference.
    This is the original full-page calendar/list implementation of the Tasks page,
    replaced by the CRM-style kanban + calendar rewrite in `designer.tasks.index`.
    Not included/rendered anywhere; safe to remove once the rewrite is fully verified.
--}}
...
<div class="crm-page-header mb-4">
    <div>
        <h1 class="text-lg font-semibold text-[var(--crm-text)]">{{ __('tasks.title') }}</h1>
        <p class="text-sm text-[var(--crm-muted)] mt-0.5">{{ __('tasks.subtitle') }}</p>
    </div>
    <div class="flex gap-2">
        <button id="view-calendar" type="button" class="filter-btn active crm-btn-sm">
            {{ __('dashboard.calendar') }}
        </button>
        <button id="view-list" type="button" class="filter-btn crm-btn-sm">
            {{ __('dashboard.list') }}
        </button>
    </div>
</div>
...
window.CrmProjectBridge = {
    ...
    onChecklistDetailClosed() {
        ...
        if (typeof window.__tasksReloadEvents === 'function') {
            window.__tasksReloadEvents();
        }
    },
};
</script>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function renderTaskMini(task, { showTime = true } = {}) {
        ...
        const isChecklist = task?.event_type === 'checklist_step' || task?.source_type === 'checklist_step';
        const checklistAttrs = isChecklist
            ? ` data-checklist-event="1" data-project-id="${Number(task.project_id || 0)}" data-checklist-id="${Number(task.project_stage_id || 0)}" data-item-id="${Number(task.source_id || 0)}" data-event-date="${escapeHtml(task.date || '')}"`
            : '';

        const icon = eventIconHtml(task);
        const doneMark = doneCheckHtml(task);
        return `
            <a href="${href}" class="event flex items-start gap-2 ${task?.done ? 'done' : ''}" style="padding:0.25rem 0.4rem; margin-bottom:0.25rem;"${checklistAttrs}>
                ${icon}
        ...
        return `
            <div class="p-3 border border-[#7c8799] dark:border-[#3E3E3A] rounded-lg bg-[#f8fafc] dark:bg-[#0a0a0a]">
                <div class="flex items-start gap-3">
                    ...
                    <div class="min-w-0 flex-1">
                        <div class="flex items-start justify-between gap-2">
                            <a href="${href}" class="font-medium text-[#0f172a] dark:text-[#EDEDEC] hover:underline break-words"${checklistAttrs}>${title}</a>
                            ${doneMark}
                        </div>
    ...
    function clearChecklistDeepLinkParams(url) {
        ...
        // keep date for calendar context
        return url;
    }

    window.__tasksClearChecklistDeepLink = function clearChecklistDeepLink() {
    ...
    window.__tasksReloadEvents = function () {
        renderCalendar();
    };
    ...
    async function openChecklistFromCalendar({ projectId, checklistId, itemId, date, pushUrl = true }) {
        ...
        const ok = await window.CrmChecklists.openByIds({
            projectId,
            checklistId,
            stepId: itemId || null,
        });
    ...
    document.addEventListener('click', (e) => {
        const link = e.target.closest('[data-checklist-event="1"]');
        if (!link) return;
        e.preventDefault();
        const projectId = Number(link.dataset.projectId || 0);
        const checklistId = Number(link.dataset.checklistId || 0);
        const itemId = Number(link.dataset.itemId || 0);
        const date = link.dataset.eventDate || '';
        if (!projectId || !checklistId) return;
        openChecklistFromCalendar({ projectId, checklistId, itemId: itemId || null, date, pushUrl: true });
    });
    ...
    async function bootChecklistDeepLink() {
