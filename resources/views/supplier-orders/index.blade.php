...
<div class="mb-6 flex flex-col md:flex-row gap-4">
    ...
    <div class="w-full md:w-48">
        <select id="project-filter" class="w-full px-4 py-2 rounded-lg border border-[#e2e8f0] dark:border-[#3E3E3A] bg-white dark:bg-[#161615] text-[#0f172a] dark:text-[#EDEDEC] focus:outline-none focus:border-[#f59e0b]">
    ...
    <div class="w-full md:w-48">
        <select id="supplier-filter" class="w-full px-4 py-2 rounded-lg border border-[#e2e8f0] dark:border-[#3E3E3A] bg-white dark:bg-[#161615] text-[#0f172a] dark:text-[#EDEDEC] focus:outline-none focus:border-[#f59e0b]">
    ...
    <div class="bg-white dark:bg-[#161615] border border-[#e2e8f0] dark:border-[#3E3E3A] rounded-lg overflow-hidden">
        <div class="overflow-x-auto">
            <table class="w-full">
                ...
                <tbody id="orders-table-body" class="divide-y divide-[#e2e8f0] dark:divide-[#3E3E3A]">
...
<div id="view-order-modal" class="fixed inset-0 bg-black/50 z-50 hidden modal-overlay" onmousedown="if(event.target === this) closeViewOrderModal()">
    <div class="absolute right-0 top-0 h-full w-full max-w-lg bg-white dark:bg-[#161615] border-l border-[#e2e8f0] dark:border-[#3E3E3A] shadow-2xl transform transition-transform duration-300 translate-x-full modal-content" onclick="event.stopPropagation()">
        <div class="flex flex-col h-full">
            <div class="flex items-center justify-between px-6 py-5 border-b border-[#e2e8f0] dark:border-[#3E3E3A] bg-[#f8fafc] dark:bg-[#0a0a0a]">
                ...
                <button onclick="closeViewOrderModal()" class="p-2 rounded-lg text-[#64748b] dark:text-[#A1A09A] hover:bg-[#e2e8f0] dark:hover:bg-[#3E3E3A] hover:text-[#0f172a] dark:hover:text-[#EDEDEC] transition-colors">
...
<div id="order-modal" class="fixed inset-0 bg-black/50 z-50 hidden flex items-center justify-center modal-overlay p-4" onmousedown="if(event.target === this) closeOrderModal()">
    <div class="bg-white dark:bg-[#161615] rounded-xl max-w-2xl w-full mx-auto max-h-[90vh] overflow-hidden flex flex-col modal-content border border-[#e2e8f0] dark:border-[#3E3E3A]" onclick="event.stopPropagation()">
        <div class="flex items-start justify-between px-6 pt-6 pb-4 border-b border-[#e2e8f0] dark:border-[#3E3E3A] shrink-0">
            ...
            <button type="button" onclick="closeOrderModal()" class="p-2 rounded-lg text-[#64748b] dark:text-[#A1A09A] hover:bg-[#e2e8f0] dark:hover:bg-[#3E3E3A] hover:text-[#0f172a] dark:hover:text-[#EDEDEC] transition-colors">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg>
            </button>
        </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    window.renderList = function() {
        ...
        listBody.innerHTML = filtered.map(order => {
            ...
            return `
                <div class="bg-white dark:bg-[#161615] border border-[#e2e8f0] dark:border-[#3E3E3A] rounded-lg p-6" data-order-id="${order.id}" data-order='${JSON.stringify(order).replace(/'/g, "&#39;")}'>
                ...
                <div class="funnel-card" draggable="true"
                    ...
                    <div class="funnel-card-actions" onclick="event.stopPropagation()">
                        <button type="button" onclick="event.stopPropagation();viewOrder(${order.id})"
                            class="text-xs px-2 py-1 rounded border border-[#e2e8f0] dark:border-[#3E3E3A] text-[#64748b] dark:text-[#A1A09A] hover:border-[#f59e0b] hover:text-[#f59e0b] transition-colors shrink-0">${viewLabel}</button>
                        <button type="button" onclick="event.stopPropagation();editOrder(${order.id})"
                            class="text-xs px-2 py-1 rounded border border-[#e2e8f0] dark:border-[#3E3E3A] text-[#64748b] dark:text-[#A1A09A] hover:border-[#f59e0b] hover:text-[#f59e0b] transition-colors shrink-0">${editLabel}</button>
                        <button type="button" onclick="event.stopPropagation();deleteOrder(${order.id})"
                            class="text-xs px-2 py-1 rounded border border-[#e2e8f0] dark:border-[#3E3E3A] text-red-500 hover:border-red-500 transition-colors shrink-0">${deleteLabel}</button>
                    </div>
                </div>
            `).join('');
        });
...
function editOrder(id) {
    ...
    if (order) {
        ...
        linksContainer.innerHTML = links.length ? links.map(l => {
            ...
            return `<div class="flex gap-2"><input type="url" name="links[]" value="${v}" placeholder="{{ __('supplier-orders.paste_link') }}" class="modal-input flex-1"><button type="button" onclick="this.parentElement.remove()" class="px-3 py-2 rounded-lg border border-[#e2e8f0] dark:border-[#3E3E3A] text-[#64748b] dark:text-[#A1A09A] hover:bg-red-50 dark:hover:bg-red-900/20 hover:text-red-500">×</button></div>`;
        }).join('') + `<div class="flex gap-2"><input type="url" name="links[]" placeholder="{{ __('supplier-orders.paste_link') }}" class="modal-input flex-1"><button type="button" onclick="this.parentElement.remove()" class="px-3 py-2 rounded-lg border border-[#e2e8f0] dark:border-[#3E3E3A] text-[#64748b] dark:text-[#A1A09A] hover:bg-red-50 dark:hover:bg-red-900/20 hover:text-red-500">×</button></div>` :
            `<input type="url" name="links[]" placeholder="{{ __('supplier-orders.paste_link') }}" class="modal-input">`;
    }
}
